    ...
    checkButtonDisabled = () => {
        ...
        if(issetNotEmpty(FORMDATA.ledger_id) && issetNotEmpty(FORMDATA.vou_date) && issetNotEmpty(FORMDATA.vouno)  && issetNotEmpty(FORMDATA.process_id))
        {
            var selectedItems = _.filter(FORMDATA.fabric_inward_inventory, (item) => {
                // console.log(item)
                return item.selected && item.fabric_id && item.color_id && item.gsm && item.dia && item.roll &&item.weight  ;
            });
                             ...
                             <thead >
                                ...
                                <tbody>  <Form.List name="fabric_inward_inventory">
                                    { (fields, { add, remove } )=> (
                                        ...
                                        <td> <Selectbox noPlaceholder withoutMargin required="false" className="col-md-12" showLabel={false} field={field} fieldKey={[ field.fieldKey, 'color_id' ]} modelName={[field.name, 'color_id']} value={[field.name, 'color_id']} disabled options={this.state.color_data} label="Color"></Selectbox></td>
                                        <td> <Numberbox noPlaceholder withoutMargin required="false" className="col-md-12" showLabel={false} field={field} fieldKey={[ field.fieldKey, 'gsm' ]} disabled modelName={[field.name, 'gsm']} value={[field.name, 'gsm']} label="Gsm"></Numberbox></td>
                                        <td>  <Numberbox noPlaceholder withoutMargin required="false" className="col-md-12"  showLabel={false} field={field} fieldKey={[ field.fieldKey, 'dia' ]} disabled required = 'false' modelName={[field.name, 'dia']} value={[field.name, 'dia']} label="Dia"></Numberbox></td>

                                        <td> <Numberbox noPlaceholder withoutMargin required="false" className="col-md-12" showLabel={false} field={field} fieldKey={[ field.fieldKey, 'roll' ]}  modelName={[field.name, 'roll']} value={[field.name, 'roll']} max={this.state.formData.fabric_inward_inventory[index]['max_roll']} label="Roll" onChange={(ev) => this.setTOTAL(ev,field.fieldKey)}></Numberbox></td>

                                        <td><Numberbox noPlaceholder withoutMargin required="false" className="col-md-12"  showLabel={false} field={field} fieldKey={[ field.fieldKey, 'weight' ]}  modelName={[field.name, 'weight']} value={[field.name, 'weight']} label="Weight" max={this.state.formData.fabric_inward_inventory[index]['max_weight']} onChange={(ev) => this.setTOTAL(ev,field.fieldKey)}></Numberbox></td>
