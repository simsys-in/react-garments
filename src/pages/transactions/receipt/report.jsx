...
class Report extends PureComponent {
    ...
    componentDidMount = () => {
        getRequest('garments/getReceiptReport?id=' + this.props.itemId).then(data => {
            if(data.status === "info")
            {
                    ...
                    //  style={{border:"1px solid gray"}}
                     >
                        ...
                        <table width={"100%"} style={{border:"1px solid gray", margin : 0, padding : 0,borderLeft:'0',borderRight:'0'}} >
                                <tr> 
                                    <td colSpan={2} style={{ backgroundColor : 'lightgray', textAlign: 'center', border : '1px solid grey', borderLeft:'0' }}> <h5>RECEIPT VOUCHER</h5> </td>
                                </tr>
                            ...
                            <div className="col-md-12" style={{ padding : 0 , borderRight:'1px solid grey'}}>
                                <table  width="100%"  >
                                    ...
                                    { report_details.inventory.map((item, index) => 
                                            <tr key={index}>
                                                <td style={{ paddingTop: item.narration === "" &&  item.account_ledger === " " ? '27px' : 'auto', paddingLeft : '5px' , borderLeft  : '1px solid grey'}}>{  item.account_ledger  + " " +  item.narration   }</td> 
                        ...
                        <div className="row">
                            <div className="col-md-12" style={{ border : '1px  solid gray', padding : 0, paddingLeft : 5,borderTop:'0' }}>
                                ...
                                {/* <p style={{ whiteSpace : 'pre-line' }}> Payment should be made by DD/pay order/Cheque or  RTGS in favour of "<b style={{fontWeight:"bold"}}>{ this.state.company_details.company }</b>"</p> */}
                                <p style={{ whiteSpace : 'pre-line',borderTop:'0' }}> Rupees in Words :  { humanize(Number(this.state.inventory_amount_total))}</p>
